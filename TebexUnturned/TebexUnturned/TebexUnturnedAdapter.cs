...
{
    public static TebexUnturned Plugin { get; private set; }

    public TebexUnturnedAdapter(TebexUnturned plugin)
    {
        Plugin = plugin;
    }
    ...
    public override void Init()
    {
        ...
        Plugin.PluginTimers().Every(61.0f, () => { DeleteExecutedCommands(); });
        Plugin.PluginTimers().Every(61.0f, () => { ProcessJoinQueue(); });
        Plugin.PluginTimers().Every(60.0f * 15 + 1.0f, () => // Every 15 minutes for store info
        {
            RefreshStoreInformation();
        });

        Plugin.PluginTimers().Every(0.5f, () =>
        {
            var task = Plugin.WebRequests().ProcessNextRequestAsync();
    ...
    public override void ReplyPlayer(object player, string message)
    {
        ...
        {
            var consolePlayer = (ConsolePlayer)player;
    ...
    public override void ExecuteOfflineCommand(TebexApi.Command command, object playerObj, string commandName, string[] args)
    {
        ...
        var player = (playerObj as SteamPlayer).ToUnturnedPlayer();
        var executer = new ConsolePlayer();
        TaskDispatcher.RunAsync(() =>
        {
            if (command.Conditions.Delay > 0)
                Thread.Sleep(command.Conditions.Delay * 1000);

            var success = R.Commands.Execute(executer, fullCommand);
            if (success)
    ...
    public override bool ExecuteOnlineCommand(TebexApi.Command command, object playerObj, string commandName, string[] args) =>
        ExecuteServerCommand(command, playerObj, commandName, args);

    public override bool IsPlayerOnline(string playerRefId)
    {
        var player = GetPlayerRef(playerRefId);
        if (player is UnturnedPlayer)
            return true;
    ...
    {
        var steamPlayer = (SteamPlayer)playerObj;
    ...
    public override void MakeWebRequest(
        ...
        TebexApi.ServerErrorCallback onServerError)
    {
        ...
        Plugin.WebRequests().Enqueue(endpoint, body, (code, response) =>
        {
            ...
            {
                ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Internal server error from Plugin API", new() { { "request", body }, { "response", response } }));
                LogDebug("Internal Server Error from Tebex API. Please try again later. Error details follow below.");
                LogDebug(response);
            ...
            {
                ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Request timeout to Plugin API", new() { { "request", body }, { "response", response } }));
            ...
            else // response is a general failure error message in a json formatted response from the api
            {
                try
                {
                    ...
                    if (error != null)
                    {
                        ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Plugin API reported general failure",
                            new() { { "request", body }, { "error", error.ErrorMessage } }));

                        onApiError?.Invoke(error);
                    }
                    else
                    {
                        ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Plugin API error could not be interpreted!",
                            new() { { "request", body }, { "response", response } }));
    ...
    public override TebexTriage.AutoTriageEvent FillAutoTriageParameters(TebexTriage.AutoTriageEvent partialEvent)
    {
        ...
        partialEvent.PluginVersion = TebexUnturned.GetPluginVersion();
