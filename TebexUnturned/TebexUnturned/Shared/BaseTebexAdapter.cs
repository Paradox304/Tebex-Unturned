...
public abstract class BaseTebexAdapter
{
    ...
    private static readonly Lazy<BaseTebexAdapter> _adapterInstance = new();
    ...
    private static readonly List<TebexApi.TebexJoinEventInfo> _eventQueue = new();

    /**
     * For storing successfully executed commands and deleting them from API
     */
    protected static readonly List<TebexApi.Command> ExecutedCommands = new();
    ...
    public static TebexConfig PluginConfig { get; set; } = new();
    ...
    public void DeleteExecutedCommands(bool ignoreWaitCheck = false)
    {
        ...
        }, error => { LogDebug($"Failed to flush completed commands: {error.ErrorMessage}"); }, (code, body) =>
        {
    ...
    public void FetchStoreInfo(FetchStoreInfoResponse response)
    {
        ...
        else
        {
            TebexApi.Instance.Information((code, body) =>
            {
                ...
                if (storeInfo == null)
                {
                    ReportAutoTriageEvent(new());
                    LogError("Failed to parse fetched store information: ");
                    LogError(body);
                    return;
                }

                Cache.Instance.Set("information", new(storeInfo, PluginConfig.CacheLifetime));
    ...
    public void RefreshListings(TebexApi.ApiSuccessCallback onSuccess = null)
    {
        ...
        TebexApi.Instance.GetListing((code, body) =>
        {
            ...
            Cache.Instance.Set("categories", new(response.categories, PluginConfig.CacheLifetime));
            if (onSuccess != null)
        ...
        TebexApi.Instance.GetAllPackages(true, (code, body) =>
        {
            ...
            Cache.Instance.Set("packages", new(response, PluginConfig.CacheLifetime));
            ...
            Cache.Instance.Set("packageShortCodes", new(shortCodes, PluginConfig.CacheLifetime));
            onSuccess?.Invoke(code, body);
        });
    }
     ...
     */
    public void GetCategories(GetCategoriesResponse onSuccess, TebexApi.ServerErrorCallback onServerError = null)
    {
     ...
     */
    public void GetPackages(GetPackagesResponse onSuccess, TebexApi.ServerErrorCallback onServerError = null)
    {
        try
        {
            ...
            else
            {
                ...
                RefreshListings((code, body) => { onSuccess.Invoke((List<TebexApi.Package>)Cache.Instance.Get("packages").Value); });
            }
        }
        catch (Exception e)
        {
            ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Raised exception while refreshing packages",
                new() { { "cacheHasValid", Cache.Instance.HasValid("packages").ToString() }, { "error", e.Message }, { "trace", e.StackTrace } }));
    ...
    public void ProcessJoinQueue(bool ignoreWaitCheck = false)
    {
        ...
        if (_eventQueue.Count > 0)
        {
            ...
            {
                ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("API error while processing join queue", new() { { "error", error.ErrorMessage } }));
                LogError($"Could not process join queue - error response from API: {error.ErrorMessage}");
            }, (code, body) =>
            {
                ReportAutoTriageEvent(
                    TebexTriage.CreateAutoTriageEvent("Server error while processing join queue", new() { { "code", code.ToString() }, { "responseBody", body } }));
    ...
    public bool CanProcessNextCommandQueue() => DateTime.Now > _nextCheckCommandQueue;

    public bool CanProcessNextDeleteCommands() => DateTime.Now > _nextCheckDeleteCommands;

    public bool CanProcessNextJoinQueue() => DateTime.Now > _nextCheckJoinQueue;

    public bool CanProcessNextRefresh() => DateTime.Now > _nextCheckRefresh;
    ...
    public void ProcessCommandQueue(bool ignoreWaitCheck = false)
    {
        ...
        TebexApi.Instance.GetCommandQueue((cmdQueueCode, cmdQueueResponseBody) =>
        {
            ...
            if (response.Meta != null && response.Meta.ExecuteOffline)
            {
                ...
                TebexApi.Instance.GetOfflineCommands((code, offlineCommandsBody) =>
                {
                    ...
                    {
                        ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Failed to parse offline commands response body",
                            new() { { "code", code.ToString() }, { "responseBody", offlineCommandsBody } }));
                    ...
                    foreach (var command in offlineCommands.Commands)
                    {
                ...
                {
                    ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Error response from API while processing offline commands",
                        new() { { "error", error.ErrorMessage }, { "errorCode", error.ErrorCode.ToString() } }));
                ...
                {
                    ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Server error from API while processing offline commands",
                        new() { { "code", offlineComandsCode.ToString() }, { "responseBody", offlineCommandsServerError } }));
            ...
            foreach (var duePlayer in response.Players)
            {
                ...
                TebexApi.Instance.GetOnlineCommands(duePlayer.Id, (onlineCommandsCode, onlineCommandsResponseBody) =>
                {
                    LogDebug(onlineCommandsResponseBody);
                    var onlineCommands = JsonConvert.DeserializeObject<TebexApi.OnlineCommandsResponse>(onlineCommandsResponseBody);
                    if (onlineCommands == null)
                    {
                        ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("Could not parse response from API while processing online commands of player",
                            new() { { "playerName", duePlayer.Name }, { "code", onlineCommandsCode.ToString() }, { "responseBody", onlineCommandsResponseBody } }));
                    ...
                    {
                        var playerRef = GetPlayerRef(onlineCommands.Player.Id);
                        if (playerRef == null)
                        {
                ...
                {
                    ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("API responded with error while processing online player's commands",
                        new() { { "playerName", duePlayer.Name }, { "code", tebexError.ErrorCode.ToString() }, { "message", tebexError.ErrorMessage } }));
        ...
        {
            ReportAutoTriageEvent(TebexTriage.CreateAutoTriageEvent("API responded with error while getting due players",
                new() { { "code", tebexError.ErrorCode.ToString() }, { "message", tebexError.ErrorMessage } }));
 ...
 */
    public void CreateCheckoutUrl(string playerName, TebexApi.Package package, CreateCheckoutUrlResponse success, TebexApi.ApiErrorCallback error)
    {
    ...
    public void BanPlayer(string playerName, string playerIp, string reason, TebexApi.ApiSuccessCallback onSuccess, TebexApi.ApiErrorCallback onError)
    {
        TebexApi.Instance.CreateBan(reason, playerIp, playerName, onSuccess, onError);
    }

    public void GetUser(string userId, TebexApi.ApiSuccessCallback onSuccess = null, TebexApi.ApiErrorCallback onApiError = null, TebexApi.ServerErrorCallback onServerError = null)
    {
        TebexApi.Instance.GetUser(userId, onSuccess, onApiError, onServerError);
    }

    public void GetActivePackagesForCustomer(
        string playerId, int? packageId = null, TebexApi.ApiSuccessCallback onSuccess = null, TebexApi.ApiErrorCallback onApiError = null,
    ...
    public abstract void MakeWebRequest(
        string endpoint, string body, TebexApi.HttpVerb verb, TebexApi.ApiSuccessCallback onSuccess, TebexApi.ApiErrorCallback onApiError,
    ...
    public void ReportAutoTriageEvent(TebexTriage.AutoTriageEvent autoTriageEvent)
    {
        ...
        if (Cache.Instance.HasValid("information"))
        {
            var storeInfo = (TebexApi.TebexStoreInfo)Cache.Instance.Get("information").Value;
        ...
        MakeWebRequest(TebexApi.TebexTriageUrl, JsonConvert.SerializeObject(autoTriageEvent), TebexApi.HttpVerb.POST,
            (code, body) => { LogDebug("Successfully submitted auto triage event"); }, error => { LogDebug("Triage API responded with error: " + error.ErrorMessage); },
            (code, body) => { LogDebug("Triage API encountered a server error while submitting triage event: " + body); });
    }
    ...
    public void ReportManualTriageEvent(TebexTriage.ReportedTriageEvent reportedTriageEvent, TebexApi.ApiSuccessCallback onSuccess, TebexApi.ServerErrorCallback onError)
    {
        ...
        MakeWebRequest(TebexApi.TebexTriageUrl, JsonConvert.SerializeObject(reportedTriageEvent), TebexApi.HttpVerb.POST, (code, body) =>
        {
            LogDebug("Successfully submitted manual triage event");
            onSuccess(code, body);
        }, error => { LogDebug("Triage API responded with error: " + error.ErrorMessage); }, (code, body) =>
        {
    ...
    public class Cache
    {
        private static readonly Lazy<Cache> _cacheInstance = new(() => new());
        private static readonly Dictionary<string, CachedObject> _cache = new();
        ...
        public bool HasValid(string key) => _cache.ContainsKey(key) && !_cache[key].HasExpired();
    ...
    public class CachedObject
    {
        ...
        public bool HasExpired() => DateTime.Now > _expires;
    }
}